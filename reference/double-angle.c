...
/* KC changed to outflow 02-16-2021
...
event init (t = 0) {
  ...
  foreach_vertex(){
     ...
     phi[] = (tan(0.314159)*(x - 3) + 1 -y)*-1. ;
    }
#endif

  fractions (phi, cms, fms);
  boundary({f,cms,fms});
  /*
  foreach()
    u.x[] = - f[];
...
event output (i+=10){
  char str[99];
  view(fov = 20, width = 1080, height = 1080,tx = -.25, ty = -0.15);
  vector h_sol[];
  #if 1
  foreach(){
     h_sol.x[]=nodata;
  ...
  /*draw_vof("csv","zeroo");
  draw_vof("csv","zeroo", filled = -1, fc = {0.3, 0.3, 0.3});*/
